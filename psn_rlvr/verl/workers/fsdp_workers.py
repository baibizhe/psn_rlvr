...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def __init__(self, config: DictConfig, role: str, **kwargs):
        ...
        # normalize config
        if self._is_actor:
            # Optional: use explicitly set normalized mini batch size (e.g. actor.ppo_mini_batch_size_normalized=36)
            ppo_mini_batch_size_normalized = self.config.actor.get("ppo_mini_batch_size_normalized", None)
            if ppo_mini_batch_size_normalized is not None:
                self.config.actor.ppo_mini_batch_size = int(ppo_mini_batch_size_normalized)
            else:
    ...
    def set_noise_sigma(self, new_sigma: float):
        ...
        if rollout is not None and hasattr(rollout, "set_noise_sigma"):
            rollout.set_noise_sigma(float(new_sigma))
            if self.rank == 0:
                logger.info(f"[ActorRolloutRefWorker] Updated rollout noise sigma to {float(new_sigma):.6f}")
        else:
            if self.rank == 0:
                logger.warning("[ActorRolloutRefWorker] rollout.set_noise_sigma not available.")
