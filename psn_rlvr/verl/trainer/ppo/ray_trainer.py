...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        self.use_param_noise = self.config.actor_rollout_ref.rollout.get("use_param_noise", False)
        self.adaptive_noise = self.config.actor_rollout_ref.actor.get("adaptive_noise", False)
        if self.adaptive_noise:
            cfg = self.config.actor_rollout_ref.actor
            self.noise_target_kl = cfg.get("adaptive_noise_target_kl", 0.005)
            self.noise_coeff = cfg.get("adaptive_noise_coeff", 1.01)
            self.noise_min_sigma = cfg.get("adaptive_noise_min_sigma", 1e-6)
            self.noise_max_sigma = cfg.get("adaptive_noise_max_sigma", 0.5)
            self.current_sigma = cfg.get(
                "adaptive_noise_initial_sigma",
                self.config.actor_rollout_ref.rollout.get("noise_sigma", 0.0),
            )
            print(
                "\n"
                + "=" * 70
                + f"\n[Adaptive Noise] ENABLED. Target Noisy_KL={self.noise_target_kl}, "
                + f"Initial Sigma={self.current_sigma}\n"
                + "=" * 70
                + "\n"
            )
        else:
            self.current_sigma = self.config.actor_rollout_ref.rollout.get("noise_sigma", 0.0)
        if self.use_param_noise:
            print(
                "[Noise Debug] rollout param-noise enabled | "
                f"adaptive={self.adaptive_noise} | sigma={self.current_sigma} | "
                f"target_modules={self.config.actor_rollout_ref.rollout.get('noise_target_modules', None)} | "
                f"base_seed={self.config.actor_rollout_ref.rollout.get('param_noise_base_seed', None)}"
            )
        else:
            print("[Noise Debug] rollout param-noise disabled")
    ...
    def _push_noise_sigma(self):
        if not self.use_param_noise:
            return
        if self.actor_rollout_wg is None:
            return
        try:
            self.actor_rollout_wg.set_noise_sigma(self.current_sigma)
            print(f"[Noise Debug] pushed rollout sigma={self.current_sigma} at global_step={self.global_steps}")
        except Exception as e:
            print(f"Warning: failed to set rollout noise sigma: {e}")
    ...
    def _validate(self, merged: bool = False):
        # Validation should evaluate clean policy behavior; when rollout param-noise is enabled,
        # temporarily set sigma to 0 and re-sync rollout weights before generating.
        restore_sigma: Optional[float] = None
        switched_to_clean_rollout = False
        if self.use_param_noise and not self.async_rollout_mode and self.current_sigma > 0.0:
            try:
                restore_sigma = float(self.current_sigma)
                print(
                    "[Noise Debug][Validation] switching rollout to clean mode "
                    f"(sigma: {restore_sigma} -> 0.0) at global_step={self.global_steps}"
                )
                self.actor_rollout_wg.set_noise_sigma(0.0)
                self.checkpoint_manager.update_weights()
                switched_to_clean_rollout = True
                print("[Noise Debug][Validation] clean rollout weights synced")
            except Exception as e:
                print(f"Warning: failed to switch to clean rollout for validation: {e}")
        ...
        try:
            ...
            if switched_to_clean_rollout and restore_sigma is not None:
                try:
                    print(
                        "[Noise Debug][Validation] restoring rollout noisy mode "
                        f"(sigma: 0.0 -> {restore_sigma}) at global_step={self.global_steps}"
                    )
                    self.actor_rollout_wg.set_noise_sigma(restore_sigma)
                    self.checkpoint_manager.update_weights()
                    print("[Noise Debug][Validation] noisy rollout weights re-synced")
                except Exception as e:
                    print(f"Warning: failed to restore rollout noise sigma after validation: {e}")
    ...
    def fit(self):
        ...
        for epoch in range(current_epoch, self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                # print('1414 gen_batch',gen_batch)
                ...
                with marked_timer("step", timing_raw):
                    ...
                    # print(1438,'gen_batch_output',gen_batch_output)
                    ...
                    if bypass_recomputing_logprobs:  # Use `rollout_log_probs`
                        ...
                        with marked_timer("old_log_prob", timing_raw, color="blue"):
                            ...
                            if "rollout_log_probs" in batch.batch.keys():
                                noisy_lp = batch.batch["rollout_log_probs"]
                                clean_lp = batch.batch["old_log_probs"]
                                if noisy_lp.shape == clean_lp.shape:
                                    response_mask = batch.batch["response_mask"]
                                    noisy_kl = noisy_lp - clean_lp
                                    mean_noisy_kl = masked_mean(noisy_kl, response_mask).item()
                                    metrics["noise/noisy_kl"] = mean_noisy_kl

                                    if self.adaptive_noise:
                                        target = self.noise_target_kl
                                        coeff = self.noise_coeff
                                        prev_sigma = self.current_sigma
                                        adjusted_sigma = self.current_sigma
                                        if mean_noisy_kl > target * 1.1:
                                            adjusted_sigma /= coeff
                                        elif mean_noisy_kl < target / 1.1:
                                            adjusted_sigma *= coeff
                                        adjusted_sigma = max(
                                            self.noise_min_sigma, min(self.noise_max_sigma, adjusted_sigma)
                                        )
                                        if adjusted_sigma != self.current_sigma:
                                            self.current_sigma = adjusted_sigma
                                            self._push_noise_sigma()
                                            direction = "decrease" if adjusted_sigma < prev_sigma else "increase"
                                            print(
                                                "[Noise Debug][Adaptive] "
                                                f"{direction} sigma: {prev_sigma} -> {adjusted_sigma} | "
                                                f"noisy_kl={mean_noisy_kl:.6f} target={target} step={self.global_steps}"
                                            )
                                        elif self.global_steps % 20 == 0:
                                            print(
                                                "[Noise Debug][Adaptive] keep sigma "
                                                f"{self.current_sigma} | noisy_kl={mean_noisy_kl:.6f} "
                                                f"target={target} step={self.global_steps}"
                                            )

                                    if self.use_param_noise:
                                        metrics["noise/current_sigma"] = self.current_sigma
